...
Table~\ref{tab:S0} summarises the monthly coverage. January and February are the only months within the Mathar validity domain ($T \in [10,25]\,^{\circ}$C); February is represented by only 25 retained observations spread over 13 active days. Consequently the retained-observation mean conditions ($\overline{T} = 28.9\,^{\circ}$C, $\overline{\mathrm{RH}} = 30.2\%$, $\overline{P} = 986.4$\,hPa) are weighted toward the warm-season segments, and differ from the time-weighted means ($\overline{T}_{\rm tw} = 25.6\,^{\circ}$C, $\overline{\mathrm{RH}}_{\rm tw} = 31.0\%$, $\overline{P}_{\rm tw} = 987.9$\,hPa) by $-3.2\,^{\circ}$C in temperature. The regression coefficients of Table~\ref{tab:coefficients} are estimated from the retained observations, and the Kramers-Kronig evaluation of Sec.~\ref{sec:S5} uses the corresponding retained-observation mean conditions.
...
\begin{table}[htbp]
\centering
\caption{Same-condition comparison of the measured environmental coefficientswith a linear surrogate of the Mathar model fitted on identical data rows.$\Delta\alpha = \alpha^{\rm data} - \alpha^{\rm Mathar-surrogate}$;intervals are 95\% bootstrap percentile intervals.}
\label{tab:S2}
\begin{tabular}{lcccc}
\hline
...
\begin{table}[htbp]
\centering
\caption{Sensitivity of the Kramers-Kronig humidity-coefficient prediction to integration range and atmospheric parameters. All integrations use a quadrature subinterval limit of 2000, verified to converge (relative error below 1\%). The measured $\alpha_H$ is $-1.3152 \times 10^{-8}\,\text{\%RH}^{-1}$; the Mathar surrogate fitted on identical data rows (Sec.~\ref{sec:S2}) is $-1.7486 \times 10^{-8}\,\text{\%RH}^{-1}$.}
\label{tab:S5}
\begin{tabular}{lccc}
\hline
...
\textit{Interpretation.} The Kramers-Kronig prediction consistently yields a negative humidity coefficient of order $10^{-8}$~\%RH$^{-1}$, confirming that the sign and rough magnitude of the measured humidity response are compatible with water-vapor dispersion. At the mean experimental conditions the prediction is $-1.35 \times 10^{-8}$~\%RH$^{-1}$ (1600--1950~nm range), close to the measured coefficient ($-1.32 \times 10^{-8}$~\%RH$^{-1}$, agreement within 3\%) and smaller in magnitude than the Mathar surrogate ($-1.75 \times 10^{-8}$~\%RH$^{-1}$). The line-by-line HITRAN2020 water-vapor spectrum therefore does not predict an enhancement of the humidity response over the broadband model at \SI{1762}{\nano\meter}. This is consistent with the same-condition comparison of Sec.~\ref{sec:S2}, where the measured humidity coefficient is found to be smaller in magnitude than the Mathar surrogate rather than enhanced. The remaining difference between the measured coefficient and the spectroscopic prediction is comparable to or smaller than the sensor gain systematic (Table~\ref{tab:S1b}); given this unresolved systematic, a quantitative attribution of the residual difference to a wavelength-specific physical effect is not possible with the present data.
